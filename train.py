...
import deepspeed
DS_CONFIG = "ds_z2_offload_config1.json"
from typing import Optional, List, Union
import sys



model_name = "/home/fanzhengbo/qwen3.59b-base"
...
ds = load_dataset("FreedomIntelligence/medical-o1-reasoning-SFT", "zh",cache_dir = './data/reasoning',split="train[:1000]")
...
# )
# 先获取对话列表
mapped_ds = ds.map(generate_conversation, batched=True)
conversations_list = mapped_ds["conversations"]

# 【关键】使用列表推导式，逐个传入 apply_chat_template
...
swanlab_callback = SwanLabCallback(
    project="Qwen3.5-9B-fintune",
    experiment_name="Qwen3.5-9-Base",
    description="使用通义千问Qwen3.5-9B模型在FreedomIntelligence/medical-o1-reasoning-SFT数据集上微调。",
    config={
...
trainer = SFTTrainer(
    ...
    args = SFTConfig(
        ...
        learning_rate = 5e-6,   # 学习率（长期训练可降至2e-5）
